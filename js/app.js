...
function init() {
  // 1. Initialise Books Database in localStorage
  const savedBooks = localStorage.getItem('novelnest_books');
  if (savedBooks) {
    state.books = JSON.parse(savedBooks);
  } else {
    // Fallback to default books data (defined in books.js)
    state.books = books;
    saveBooks();
  }
  ...
  const savedUser = localStorage.getItem('novelnest_user');
  if (savedUser) {
  ...
  // 5. Render Cover Presets in Modal
  renderCoverPresets();

  // 6. Init View Router State
  switchView(state.activeView);
}

function saveBooks() {
  localStorage.setItem('novelnest_books', JSON.stringify(state.books));
}

function saveCart() {
  localStorage.setItem('novelnest_cart', JSON.stringify(state.cart));
}
...
function setupEventListeners() {
  ...
  booksGrid.addEventListener('click', (e) => {
    ...
    if (addToCartBtn) {
      ...
      // Don't open modal if clicked card is out of stock (or open anyway but read-only)
...
function handleLogin(e) {
  ...
  if (username === 'admin' && password === 'admin123') {
    ...
    localStorage.setItem('novelnest_user', JSON.stringify(state.currentUser));
    showToast('Admin logged in successfully!', 'success');
    switchView('admin');
  } else if (username === 'user' && password === 'user123') {
    state.currentUser = { username: 'Munir', role: 'customer' };
    localStorage.setItem('novelnest_user', JSON.stringify(state.currentUser));
      ...
      <div class="book-card ${isOutOfStock ? 'out-of-stock' : ''}" data-id="${book.id}">
        ...
        <div class="book-info">
          ...
          <div class="book-rating">
            ...
            <span class="rating-num">(${book.publishedYear})</span>
          </div>
...
function updateCartUI() {
  ...
  cartItemsContainer.innerHTML = state.cart.map(item => {
    ...
    return `
      <div class="cart-item">
        <div class="cart-item-cover" style="background: ${book.coverGradient}">
          ${book.title}
        </div>
...
function openDetailsModal(bookId) {
  ...
  detailsModalContent.innerHTML = `
    <div class="details-cover-container">
      <div class="book-cover-mock" style="background: ${book.coverGradient}; width: 160px; height: 240px; font-size: 1rem; padding: 1.5rem 1.25rem;">
        <div class="cover-title">${book.title}</div>
        <div class="cover-author">${book.author}</div>
      </div>
    </div>
    <div class="details-info">
      ...
      <div class="details-meta">
        <div class="meta-item">
          ...
          <span class="meta-value" style="color: #fbbf24;"><i class="fa-solid fa-star"></i> ${book.rating.toFixed(1)}</span>
        </div>
        <div class="meta-item">
      ...
      <div class="details-action">
        <span class="details-price">$${book.price.toFixed(2)}</span>
...
function submitOrder() {
  ...
  setTimeout(() => {
    ...
    const orderAddress = document.getElementById('checkout-address').value.trim();
    
    let totalAmt = 0;
    // Deduct stock levels in local storage and save!
    ...
    document.getElementById('summary-address').textContent = orderAddress;
      ...
      <tr>
        <td>
          <div class="admin-table-book">
            <div class="admin-table-cover" style="background: ${book.coverGradient}">
              ${book.title}
            </div>
            <div>
        ...
        <td><strong style="font-weight:750;">$${book.price.toFixed(2)}</strong></td>
        <td>
...
function adjustStockInline(bookId, delta) {
  const book = state.books.find(b => b.id === bookId);
  if (book) {
...
function setStockInline(bookId, newVal) {
  const book = state.books.find(b => b.id === bookId);
  if (book) {
    book.stock = newVal;
    saveBooks();
...
function openAddBookModal() {
  ...
  // Set default active preset gradient (first one)
  document.querySelectorAll('.color-preset-option').forEach(el => el.classList.remove('active'));
...
function handleAddBook(e) {
  ...
  const publishedYear = parseInt(document.getElementById('add-book-published').value);
  ...
  const coverGradient = addBookGradientVal.value || GRADIENT_PRESETS[0].value;
  ...
  // Generate ID
  const nextId = state.books.length > 0 ? Math.max(...state.books.map(b => b.id)) + 1 : 1;
